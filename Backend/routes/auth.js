...
const JWT_SEC_KEY = "jfl;jalfaournv,cvbz,v,znv,.noeljafdljaflflclaclriyeioyriwaaior";
...
router.post('/createcustomer', [
    ...
    try {
        ...
        if (customer) {
            return res.status(400).json("Already a customer exist")
        }
...
router.post('/logincustomer', [
    ...
    try {
        ...
        if (!isCustomer) { res.status(400).json({success : success, error: "Please enter the correct details" }) };
